...
import type { Snippets } from 'contentlayer/generated'

interface Props {
  snippets: CoreContent<Snippets>[]
  title: string
  initialDisplaySnippets?: CoreContent<Snippets>[]
...
export default function SnippetsLayout({
  ...
  const filteredSnippets = snippets.filter((frontMatter) => {
    const searchContent = frontMatter.title + frontMatter.summary + frontMatter.tags.join(' ')
    return searchContent.toLowerCase().includes(searchValue.toLowerCase())
  })

  const displaySnippets =
    initialDisplaySnippets.length > 0 && !searchValue ? initialDisplaySnippets : filteredSnippets
        ...
        <div className="grid grid-cols-1 gap-8 py-12 md:grid-cols-2 lg:grid-cols-3">
          ...
          {displaySnippets.map((frontMatter) => {
            ...
            return (
              ...
              >
                <div className="absolute bottom-0 left-0 h-0.5 w-full origin-left scale-x-0 transform bg-primary-500 duration-300 group-hover:scale-x-100" />
                ...
                <div className="absolute top-0 left-0 h-0.5 w-full origin-right scale-x-0 transform bg-primary-500 duration-300 group-hover:scale-x-100" />
